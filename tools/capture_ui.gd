...
func _capture_state(window_size: Vector2i, label: String, state_name: String, open_drawer: bool) -> void:
	...
	if state_name == "run" or state_name == "run_standing":
		...
		var lost := RunSummary.new(
			96, 4203, 2, ScientificNumber.from_float(142580), 1, "death",
			ScientificNumber.from_float(12300), true
		)
		lost.wave_hp_left = ScientificNumber.from_float(55475)
		lost.damage_in_timer = ScientificNumber.from_float(23100)
		lost.number_before_hit = ScientificNumber.from_float(3980)
		main._show_died_screen(lost)
	if open_drawer:
		main._toggle_drawer()
	main._refresh_all()
	await process_frame
