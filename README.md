...
- **Liability** is depleted by production, and the same output is also Number (D037): the Number never stops rising while you produce. A beaten wave gives way to the next after a 2.5-second beat; a missed wave moves on, and whatever of it reached the Number stays and keeps hitting, 4% harder each time; a boss is a wall of HP that hits like one enemy and stays at the Number while the next waves come (D058, D063).
