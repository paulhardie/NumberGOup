...
A portrait-first, local-save Godot web idle game with a tiered Wave-run foundation.
...
The playable slice includes tapping, passive production, a four-category permanent Workshop, Research Focus, upgrade synergies, Prestige, Knowledge/Insight, local saves, a stats/settings drawer, and tiered Wave runs.

Wave runs use two independent absolute stats:

- **Wave HP** is depleted by production first. Output that damages the wave does not become Number; only overflow beyond the remaining Wave HP does.
- **Hit** is deducted from Number every 15 seconds while the wave remains standing. Number reaching zero ends and resets the run.

Tier 1 has 20 warm-up waves. Tier 2 and Tier 3 unlock by clearing wave 100 in the preceding tier and apply 20× and 60× Wave HP/Hit pressure from wave 1. Rewards rise by smaller 1.8× and 2.6× multipliers. Every tenth wave is a boss; milestones are tracked at waves 10, 25, 50, and 100.
...
Retreat ends and resets a run—it cannot pause a Wave run while Number keeps growing. Active runs freeze exactly while the app is away. Save schema V5 preserves the permanent Workshop, active encounter, deterministic RNG state, category choice, and Defense unlock, with V1–V4 migration.
...
`Workshop` is the permanent machine-building layer: Attack is available from the start, Defense appears after the first Hit, Utility opens at Workshop level 8, and Ultimates shows its milestone slots from the start. `Research` softly specialises permanent Coin costs by discounting Attack, Defense, or Utility until the next Prestige rather than closing alternatives. Cards are typed as `Modules`, `Protocols`, or `Routines`. `Knowledge`, `Insight`, Coins, Armor, and Workshop ranks survive run resets. A future temporary in-run upgrade layer must use a distinct name and resource rather than blurring this boundary. The taxonomy reserves `Breakthroughs`, `Laws`, and `Violations` for future systems that will use the shared modifier pipeline.
