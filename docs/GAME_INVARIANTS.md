...
- The wave's number on screen (D050, D051) is presentation: its place is the wave clock, it reaches the Number exactly when the Hit lands, a boss that has landed stays there, and nothing about it (or the motes) feeds back into a rule or the run's random stream. Motes delay only the shown HP, never the damage.
