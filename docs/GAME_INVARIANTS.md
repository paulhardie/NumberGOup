...
- Tap Damage and Damage (Damage per Second before D054) run to 6,000 ranks and Guard to 5,000 (D047). Ranks 1–100 keep their step, so an owned rank never loses value; past 100 a rank is worth more along the row's depth curve, and never less than the rank before it. A row keeps its old prices up to its old cap, and past it each rank costs the row's deep growth more than the last. A Workshop multi-buy quotes exactly the ranks and price that single presses would.
