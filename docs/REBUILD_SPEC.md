...
- **Milestone 1, the battle alone: built, merged and played by the owner** ("looks good"; combat felt jittery, fixed in milestone 2). A fresh tower that buys nothing dies on wave 2 in about 50 seconds on every seed, matching The Tower's "you die immediately". What's in it: `tools/import_tower_enemies.mjs` and its data, `src/tower/` (data, guesses, the simulation), `src/ui/` (the battle screen), `tools/sim_runs.gd`, `tools/capture_battle.gd` and `tests/tower_tests.gd`.
- **Milestone 2, run upgrades and Cash: built and merged.** An Attack, Defense and Utility panel sells the open rows (Damage, Attack Speed, Critical Chance, Critical Factor, Health and Health Regen) at The Tower's Cash prices, priced by how many of that row the run has bought. Utility says its rows open in the Workshop. Health bought heals by its gain. Enemies and shots are drawn between their last two ticks, so movement is smooth at any frame rate. Measured with `tools/sim_runs.gd`, 10 seeds each: **a fresh run that spreads its Cash across the rows dies on wave 8 (median, range 8–10), the owner's benchmark**; only Damage and Attack Speed reaches wave 10 (a boss ends most); buying the cheapest level every time wastes Cash on Critical Chance and Health Regen and dies on wave 2; buying nothing dies on wave 2.
- **Milestone 3, the Workshop, Coins, home and saving: built, merged and played by the owner.** A home screen (Battle, Workshop, best wave, Coins), a Workshop screen buying permanent levels at The Tower's Coin prices, and groups opening in The Tower's order. The rows those groups bring now work in battle: Range, Damage / Meter, Multishot, Defense % and Absolute, Thorns, Cash Bonus, Cash / Wave, Coins / Kill Bonus and Coins / Wave. Groups past Thorns show as coming soon (milestone 4). Coins go into the Workshop as they're earned. The save is `user://number_go_up_tower.json`, version 1, written whole or not at all, and an unreadable file is moved aside, never written over. An End run button ends a run early. Careers in `tools/sim_runs.gd` (12 runs from a fresh Workshop):
...
- Enemies set off 100 m out, basic speed is 10 m a second, the base range is 30 m, and orbs circle at 60 m (D067, D068). The Tower gives no units. At 60 m the orbs are off the edge of the screen, which suggests they sit closer in The Tower.
- Orb Speed is read as turns a minute (0.4 at its first level), so orbs barely move and catch few enemies. The unit is likely wrong.
- Ranged enemies stop and fire from 30 m, exactly the base Range (D067), and end most runs in the careers.
