...
**Last updated:** 24 September 2026, after building D054 (the Workshop speaks in shots: Damage, Attack Speed, Multishot, one mote per shot) on branch `claude/game-changes-review-fbili3`, not merged. D044–D053 and the class-cache fix are merged to `main` (PRs #48–#52).
...
`main` plus the D054 branch plays like this (seed 7 simulator figures, Godot 4.7.2, balance profile `tax-foundation-v10`; nothing below has been checked on a phone):
...
1. **Play-test and merge branch `claude/game-changes-review-fbili3`** (D054: Damage, Attack Speed and Multishot, one mote per shot). Low risk: presentation only.
2. **Step 2 of the shots rework: how much tapping should add, and the damage/speed split.** Today tapping is most of a fresh run's damage (two taps a second about triples it; 0 taps reaches about wave 19, 2 a second 29, 6 a second 40) but only about a fifth by rank 100, and run Cash assumes one tap a second whether or not you tap. The proposal: a tap fires a shot worth a share of current Damage (Tap Damage becomes that share), taps past about five a second count half, and target idle 100%, one tap a second about +15–20%, three about +40–50%, a cap near double. High risk: economy; measure with the balance simulator's taps-a-second sweep and the career simulator before building.
3. **When position becomes a rule:** slowing bosses (not pushing them), and range or several bodies for area damage.
4. **The early-game pacing change.** D047's bigger Attack Speed and Crit steps halve the focused player's time to wave 100. Built as approved. The alternative keeps today's per-rank steps and adds ranks instead (Attack Speed 196, Crit Chance 320, Crit Damage 284 ranks), which keeps the first hours as they were and moves the extra power later.
5. **A wave becomes a group** ([proposal](WORKSHOP_EXPANSION.md#proposal-a-wave-becomes-a-group--24-september-2026)): after slowing lands, split a wave into 2–5 members with the same totals, staggered arrival, front-first damage, bosses single, and Guard on the group's combined Hit.
...
1. **Owner:** play D054 and merge it or ask for changes; decide the tapping proposal (decision 2). **Agent:** measure it first (taps-a-second sweep, career simulator), then build it as its own decision. High risk: economy.
...
3. **Owner:** decide the pacing alternative (decision 4). **Agent:** if the alternative is chosen, it is a data change in `data/workshop/upgrades.json` plus a price recalibration. Medium risk.
...
- **One mote per shot (D054) was checked in a scripted run and screenshots, not in motion.** At full Attack Speed and Multishot a wave takes up to twelve "-X" pops a second, and while a wave is at the top edge they rise into the header line ("WAVE 18 T -3.17 BOSS IN 2"). If that reads as clutter, the fixes are pops that rise less or fold together, not fewer motes.
