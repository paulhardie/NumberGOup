...
**Last updated:** 24 September 2026, after building D051 (the run arena: each wave's HP falling to the Number, motes, red bosses that latch on) and D052 (the raw Hit on the wave, the defences worked out at contact) on branch `claude/game-changes-review-fbili3`, not merged. D044–D050 are merged to `main` (PRs #48–#50).
...
`main` plus the D051–D052 branch plays like this (seed 7 simulator figures, Godot 4.7.2, balance profile `tax-foundation-v10`; nothing below has been checked on a phone):
...
1. **Play-test and merge branch `claude/game-changes-review-fbili3`** (D051–D052, the run arena and the Hit's working). Low risk: presentation only. Then decide when position becomes a rule: slowing bosses (not pushing them), and range or several bodies for area damage.
