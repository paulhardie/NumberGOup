...
- **Careers** (`tools/sim_runs.gd -- --careers 12`): a focused player reaches wave 22 on run 12 (1.3 hours), matching the owner's wave-22 time almost exactly. Spreading every Coin and Cash evenly stalls at waves 7–10. Kills now pay flat Coins (D074): the first dozen runs earn 50–130 Coins each, wave 20 comes on run 18 (1.7 hours).
- **Not played by the owner yet, not on a phone.**

## Open decisions for the owner

1. **Cheaper Workshop prices?** The owner finds them high. They are The Tower's own for a new account: the SDK table, the wiki and the owner's own fresh-save screen (30 for the core rows, 50 for crit) agree. The Tower lowers them later with Workshop Discount labs (0.5% a level, up to 49.5% a tab) and the Vault, and an account with progress may have those. **Recommend: keep The Tower's prices** until a screenshot of the owner's Tower Workshop (Damage's level and price) shows otherwise. Cutting them is a deliberate departure from The Tower.
...
1. **Owner:** merge flat Coins, play from a fresh save for a few runs, and send a screenshot of The Tower's Workshop Attack tab (decision 1). **Done when:** the owner says whether the Workshop feels like The Tower's early game.
