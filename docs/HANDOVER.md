...
**Last updated:** 24 September 2026, by Codex, handing back to Claude. Branch `claude/game-changes-review-fbili3` carries **D056–D061, including the independent D059 review, its approved saved-run repair and quieter combat readouts**. The branch is **not merged**, and no PR is open (checked with `gh pr list` during review). D044–D055 and the class-cache fix are merged to `main` (PRs #48–#54). The owner's playable checkout remains on `main`; review work is in `/private/tmp/ngu-d059-review`.
...
1. Read [`AGENTS.md`](../AGENTS.md), D056–D061 in [`DECISIONS.md`](DECISIONS.md), and the D059 implementation commit `e934e91`. Fetch and check the branch against `origin` before new work. Do not switch the owner's playable `main` checkout to this branch.
...
3. **D061 quiets the combat numbers.** Damage now updates one readout beside the target, and routine incoming Hits one readout below TAP; close guarded Hits keep only the latest working column. The arena probe and four-size capture pass, but no phone or touch play has checked the feel.
4. **The next owner decision is whether D059 and D061 feel right in play and should go to a PR.** Ask about the wave-31 switch, Hits, early Coin pacing and readout legibility. Keep the remaining pile overlap and narrow caption pass separate. Do not open or merge a PR without the owner asking. Use `run_godot.sh` for every Godot run and check for a running Godot game before editing scripts.
...
- **The arena:** the front enemy is the live number, with "hits X" and a countdown once it is at the Number. The others walk in smaller behind it, and those at the Number flank it in red, three rows a side, 18 drawn at most. A boss stays the live number even behind a pile. Motes fly at the front enemy. D061 shows damage dealt in one fixed half-second total beside the target and routine incoming Hits in one fixed half-second total below TAP. Guarded Hits still work out over the Number, one column at a time. Clears still float "BEATEN".
...
- **The look and screens (D048, D049, D053, D061):** the Instrument palette and layout remain; the combat readouts are quieter on this branch. The design canvas is https://claude.ai/artifact/8pB4uLUkva6kbnBRZ3PBXv.

## Open decisions for the owner

1. **Play and open a PR for this branch?** Recommend playing D059 with D061's readouts first, then opening its PR if the wave-31 switch, Hits, early Coin pacing and combat legibility feel right. The automated review blocker is resolved, but the owner has not played this branch and no PR is open.
...
1. **Owner:** play D059 with D061's readouts and decide whether to open its PR. **Done when:** the wave-31 switch, Hits, early Coin pacing and combat legibility feel acceptable in a real game; no local automated check proves that feel. Low technical risk, but a product decision. No PR or merge has been requested.
...
- **Arena:** `tools/arena_probe.gd` (checks and screenshots) and `tools/capture_ui.gd` (thirteen screens at four sizes, including `run_feedback`). Look at the PNGs; never assert pixel equality.
...
The approved saved-run repair and the requested quieter combat readouts are complete. The next decision is whether D059 and D061 feel right in play and should have a PR. Keep this handover as the current state, and leave the repository able to answer the next session's questions without this conversation.
...
2. **Record any new choice the owner accepts** as the next `D0NN` entry in [`DECISIONS.md`](DECISIONS.md) (D062 onwards). D059 records the opening, D060 the saved-run reconciliation and D061 the fixed combat readouts. Each new entry needs the owner's words, context, the decision, the evidence (the figures measured and how), the consequences, and when to revisit. Update any document the change makes stale in the same commit: [`GAME_INVARIANTS.md`](GAME_INVARIANTS.md), [`MOTION_SYSTEM.md`](MOTION_SYSTEM.md), [`WORKSHOP_DESIGN.md`](WORKSHOP_DESIGN.md), [`WORKSHOP_EXPANSION.md`](WORKSHOP_EXPANSION.md).
