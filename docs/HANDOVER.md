...
6. **Monetisation, parked until the game is in better shape (owner, 24 September 2026):** the owner intends to compete with The Tower and will monetise, "in a less aggressive fairer way". Nothing is decided and nothing should be built for it yet. When it is picked up, decide **what money may buy**. The recommendation was cosmetics, a one-off supporter or remove-ads purchase and possibly paid content (tiers, challenge sets), with Gems earn-only and no power, Card pulls or timer skips for sale. Before then, avoid deepening what would become unfair if Gems were sold: Gem-pulled Cards (D027) would be paid loot boxes, and Gem-bought Lab slots on real-time research (D024, D029) are the pay-for-time pattern The Tower's players complain about. Seeded runs (D006) could support challenges money cannot buy, checked by replaying the run on a server. Selling anything needs purchase receipts checked and a server, because the local save can be edited. The vision's "no store" rows and the unsettled Gem supply and prices are where this lands.
