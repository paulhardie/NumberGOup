...
**Last updated:** 24 September 2026, by Codex, handing back to Claude. Branch `claude/game-changes-review-fbili3` carries **D056–D060, including the independent D059 review and its approved saved-run repair**. The branch is **not merged**, and no PR is open (checked with `gh pr list` during review). D044–D055 and the class-cache fix are merged to `main` (PRs #48–#54). The owner's playable checkout remains on `main`; review work is in `/private/tmp/ngu-d059-review`.
...
1. Read [`AGENTS.md`](../AGENTS.md), D056–D060 in [`DECISIONS.md`](DECISIONS.md), and the D059 implementation commit `e934e91`. Fetch and check the branch against `origin` before new work. Do not switch the owner's playable `main` checkout to this branch.
...
3. **The next owner decision is whether D059 feels right in play and should go to a PR.** Ask for hands-on feedback on the wave-31 switch, Hits and early Coin pacing. No phone, touch or motion check has been done. Do not open or merge a PR without the owner asking.
4. Keep the arena layout pass separate. Its overlap, narrow caption and rising pop issues remain for the owner to confirm after D059 play. Use `run_godot.sh` for every Godot run so `user://` cannot touch the real save. Check for a running Godot game before editing scripts, as `AGENTS.md` requires.
...
1. **Play and open a PR for this branch?** Recommend playing D059 first, then opening its PR if the wave-31 switch, Hits and early Coin pacing feel right. The automated review blocker is resolved, but the owner has not played D059 and no PR is open.
...
1. **Owner:** play D059 and decide whether to open its PR. **Done when:** the wave-31 switch, Hits and early Coin pacing feel acceptable in a real game; no local automated check proves that feel. Low technical risk, but a product decision. No PR or merge has been requested.
...
- **This review and repair actually ran:** a D058-shaped V10 regression fixture failed four assertions before D060, then `bash run_tests.sh` → `PASS: economy tests` after it (the suite prints no test count; its deliberate corrupt-save fixture prints an `Exponent too high` warning). Headless boot exited 0 without errors; `tools/arena_probe.gd` → `ARENA PROBE PASS`. The windowed probe printed a shader-cache warning but completed; its approach, boss and pile PNGs were inspected during review, before D060's load-only repair. `bash run_balance.sh` completed after D060 and was byte-identical to the pre-repair D059 run: seed 7 fresh wave 20/86 Coins, early wave 30/273 Coins, and the simulator's rank-100 Attack build wave 156/9,327 Coins. These single-seed results do not replace D059's six-seed figures. CI and the career simulator were **not** rerun, and neither motion by touch nor a phone was checked.
...
The approved saved-run repair is complete. The next decision is whether D059 feels right in play and should have a PR. Keep this handover as the current state, and leave the repository able to answer the next session's questions without this conversation.
...
2. **Record any new choice the owner accepts** as the next `D0NN` entry in [`DECISIONS.md`](DECISIONS.md) (D061 onwards). D059 records the accepted opening and D060 the approved saved-run reconciliation. Each new entry needs the owner's words, context, the decision, the evidence (the figures measured and how), the consequences, and when to revisit. Update any document the change makes stale in the same commit: [`GAME_INVARIANTS.md`](GAME_INVARIANTS.md), [`MOTION_SYSTEM.md`](MOTION_SYSTEM.md), [`WORKSHOP_DESIGN.md`](WORKSHOP_DESIGN.md), [`WORKSHOP_EXPANSION.md`](WORKSHOP_EXPANSION.md).
