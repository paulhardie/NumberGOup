...
| --- | --- | --- |
| A tap with Liability left | A mote leaves the Number for the wave at once and the Number pulses; as it lands (0.32 s) the wave's HP drops and its damage updates one fixed readout beside the struck member (D061). Passive damage still sends one mote per shot (2.5 a second before Attack Speed, D055); Multishot sends two, the second 0.08 s behind (D054). The readout combines damage for 0.5 s; a critical total is heavier red with no word (D062). It rests after 0.8 s without a hit. Between waves a tap's gain floats up as "+X", red and heavier for a crit. | No motes; the shown HP and readout update when passive damage is collected every 0.33 s. Number pulse skipped. |
| Passive ticks | The arc advances only. A strike per tick would strobe, so ticks deliberately have no separate hit. | Unchanged. |
| `tax_collection` | If reduced or blocked, the Hit's working over the Number (D052): raw Hit, Guard, Armor and what landed, a line every 0.15 s, held 0.8 s and faded. Only the latest working remains when Hits arrive close together (D061). An unreduced Hit updates the single fixed "HIT -X" readout below TAP, combined with pile Hits for 0.5 s. Number flash, stage impact flash, a light stage rattle (3.5 px) and a short haptic remain. | Working, readout and colour only. |
| `boss_collection` | The same at boss weight: a 9 px rattle, a longer flash, a stronger haptic. | Readout and colour only. |
...
| The wave's number (D050, D051, D057, D067, D068) | The Number sits in the middle of the arena inside a faint ring at its reach (the Range row's), as The Tower's field is laid out, with any orbs drawn as small dots circling at their radius on the run's clock (hidden under Reduce Motion, since they are presentation of a rule the enemies' own motion already shows), and each member walks straight in from its own direction, drawn at the distance it has covered, from the time it sets off: the member the Number strikes large with its caption, the rest smaller and quieter; ranged ones stand on the ring. The enemy types that pay Coins lean towards the Coin colour, tanks are drawn larger, and the front member's caption names a type that isn't basic (D066); a member carried past its wave's clock keeps walking from where it was. Motes fly at the front member. A beaten front member scatters where it stood and the next takes its place. Members that reach the Number stay there (D058), ringing it in red in three staggered rows; repeat Hits update the fixed "HIT -X" readout and briefly flash the Number (D061). The front member's HP falls to the Number with a faint trail. A clean clear scatters its digits (0.6 s) with a "BEATEN · NO HIT" float; a Hit swells a copy into the Number (0.43 s); a boss then stays on the Number, its caption counting to the next Hit, and stays the live number in red as later waves come in behind it (D063), without fading in again. | The front member holds at the top edge with its countdown and a count of the members behind it; no followers, scatter or slam. |
