...
- **Status:** Accepted (2026-09-22) on owner direction. Implemented in full (2026-09-22): the heading, cause and doors in step 5, the two gaps in step 6.
...
- **Decision:** The screen names the cause, not the cure. It reads `LOST TO`, then the wave and whether a boss landed it, then the size of the hit that emptied the Number. Step 6 adds the two gaps underneath in the same voice — how far short Attack fell against that wave's HP and Defense against its hit — as facts rather than as prescriptions. The smaller gap points at the category to open next without saying so in words.
- **Consequences:** A counterfactual reads as a lecture and quietly tells the player what to buy; a cause tells them what happened and leaves the conclusion to them. It also fits the vision's "every number that kills you was visible before it did" more exactly, because naming the number that did it is a fact about the run rather than advice about the next one. `RunSummary` carries the killing hit, the boss flag and the final stand — the HP still standing, what the last timer dealt, and the Number the hit landed on — since `_reset_run_state` wipes the encounter before the screen can read it; a retreat records none of it, having been lost to nothing. Damage per timer is measured through `_add_number` rather than inferred from the production rate, so a player who taps hard is credited for it. The nearer miss carries the accent and the other is muted, which is the whole of pointing at a category: the two multiples are not comparable in Coins, so the smaller one is the nearer miss rather than the cheaper fix, and the screen places the two facts side by side without holding an opinion.
