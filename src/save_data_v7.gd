...
## V7 is V6 plus `lab_slots`, the number of Lab research slots open (D029).
## Every other key keeps its V6 name and meaning. A save from before V7 has
## no slot count; it predates bought slots, when every player had two, so it
## loads with two rather than the one a new game starts with.
const VERSION := 7
...
static func problem(data: Dictionary) -> String:
	...
	for key in ["coins", "knowledge", "gems", "lab_slots", "wave", "highest_wave", "selected_tier"]:
