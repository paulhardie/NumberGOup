...
- `tools/career_simulator.gd` plays a fresh save run after run, spending Coins between runs, and compares today's rules with the proposed coin gates and other run rank worths; `-- --runs N`, `-- --layout plan|tower`, `-- --ladder NAME` and `-- --careers a,b` narrow it. It is a measurement tool, not a gate.
