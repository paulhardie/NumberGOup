...
- `tools/sim_runs.gd` plays seeded runs headless and prints the wave, game time, kills, Cash and Coins each ended with, and the median; `-- --seeds N` and `-- --cap-minutes N` narrow it. It is a measurement tool, not a gate.
