...
- I play from `~/NumberGOup-main`, the play checkout. It must always be on `main` at `origin/main` once work is merged, so I never play an old build. When a game is running from it, do your work in a separate worktree, as "Protect the real save" requires; when your pull request merges, make sure the play checkout ends on the merged `main`, and tell me to quit and reopen Godot, because an open editor keeps the old scripts. Never leave the play checkout on a feature branch.
- A launchd job on my Mac (`com.paulhardie.ngu-autopull`) runs every five minutes. When the play checkout is clean and no game is running from it (a game is a Godot process started with `--path ~/NumberGOup-main`; the editor alone does not count), it fast-forwards `main` from `origin`, and if the checkout is on a branch already fully merged into `origin/main` it switches back to `main` first. It leaves an unmerged branch, a dirty tree or a diverged `main` alone, never commits, pushes or discards, and posts a Mac notification when the checkout moves. Script: `~/.local/bin/ngu-auto-pull.sh`; log: `~/Library/Logs/ngu-autopull.log`.
