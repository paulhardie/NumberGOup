...
- Take them from the real plan: the milestones in [`docs/REBUILD_SPEC.md`](docs/REBUILD_SPEC.md), the open decisions in [`docs/HANDOVER.md`](docs/HANDOVER.md), and gaps found during the work. Not generic advice like "add more tests".
...
- [`docs/REBUILD_SPEC.md`](docs/REBUILD_SPEC.md) — the rebuild (D073): The Tower's first hours built clean, its milestones, benchmarks and guesses. While the rebuild runs it is the plan.
...
- [`docs/TOWER_EARLY_PROGRESSION.md`](docs/TOWER_EARLY_PROGRESSION.md) — a reading of how The Tower's first hours reveal systems (Cards at wave 20, Labs at 30) and why the Tier 1 turtle works then breaks; the owner's reference, unchecked.
- `data/tower/enemies.json` — The Tower's Tier 1 enemies, wave by wave, generated by `tools/import_tower_enemies.mjs`.
- [`docs/archive/`](docs/archive/) — the pre-rebuild game's design documents (vision, Workshop design, motion, invariants and others), kept as history. Where they disagree with the rebuild, the rebuild wins.
...
1. **One clear authority per concept.** The Tower's numbers live in the generated data under `data/`, read through `src/tower/tower_data.gd`; the numbers we guess live in `src/tower/guesses.gd`; combat rules live in `src/tower/battle_sim.gd`.
2. **UI never owns domain logic.** The screens under `src/ui/` present and report; rules belong to `BattleSim`. A rule implemented twice is a bug report waiting to happen.
...
7. **Factor before you add.** A new domain growing inside `BattleSim` should become its own class with focused tests, as the old `TaxBalanceProfile` and `TaxEncounter` did out of `GameState`.
...
- Use the destination ([`docs/REBUILD_SPEC.md`](docs/REBUILD_SPEC.md), and [`docs/archive/GAME_VISION.md`](docs/archive/GAME_VISION.md) for the longer ambition), the reference research ([`docs/TOWER_SCALING_FOUNDATION.md`](docs/TOWER_SCALING_FOUNDATION.md)) and the current code as three independent inputs.
...
- Run Godot only through `run_godot.sh`, never the binary directly. It points `HOME` at a scratch folder (`${TMPDIR:-/tmp}/ngu-home`, created on first use), which moves `user://` and its save there; `run_tests.sh` already goes through it. The rebuilt game saves to its own file, `user://number_go_up_tower.json` (D073), so it never touches the old save; it saves after Workshop purchases, at a run's end, every 20 seconds of battle and when the window closes or loses focus. On macOS `HOME` is the only override that works; the `XDG_*` variables are ignored.
...
- `run_tests.sh` runs `tests/tower_tests.gd`; a green count with errors printed is not a pass, and the script fails the run on any error line. If it fails on classes it cannot find, the `.godot` cache is stale: run `bash run_godot.sh --headless --path . --import`.
- `tools/sim_runs.gd` plays seeded runs headless and prints the wave, game time, kills, Cash and Coins each ended with, and the median; `-- --seeds N` and `-- --cap-minutes N` narrow it, `-- --buy none|cheapest|even|attack|core` picks how it spends Cash (`core` also focuses a career's Workshop), and `-- --careers N` plays N runs in a row from a fresh Workshop, spending Coins between them. It is a measurement tool, not a gate.
...
- A launchd job on my Mac (`com.paulhardie.ngu-sync`, installed by `tools/mac/install_sync.sh`) moves my play folder, `~/NumberGOup-main`, to `origin/main` every minute, so whatever is merged is what I play. Local changes or commits there are kept on a `local-backup/<time>` branch first, never discarded. Script: `tools/mac/ngu-sync.sh`, installed at `~/.local/bin/ngu-sync.sh`; log: `~/Library/Logs/ngu-sync.log`. The older job (`com.paulhardie.ngu-autopull`) is disabled: it fast-forwarded only a clean tree, which the Godot editor often prevents.
