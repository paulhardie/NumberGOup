...
- Take them from the real plan: the roadmap in [`docs/REBUILD_SPEC.md`](docs/REBUILD_SPEC.md), the open decisions in [`docs/HANDOVER.md`](docs/HANDOVER.md), and gaps found during the work. Not generic advice like "add more tests".
...
- `tools/sim_runs.gd` plays seeded runs headless and prints the wave, game time, kills, Cash and Coins each ended with, and the median; `-- --seeds N` and `-- --cap-minutes N` narrow it, `-- --buy none|cheapest|even|attack|core` picks how it spends Cash (`core` also focuses a career's Workshop), and `-- --careers N` plays N runs in a row from a fresh Workshop, spending Coins between them, and `-- --divider-share N`, `--divider-health N`, `--divider-speed N`, `--divider-divisor N` and `--overfill N` try other tunings without changing the game; `--curve` adds the Number every fifth wave. It prints each run's peak Number and how many Dividers came and landed. It is a measurement tool, not a gate.
