...
- `.github/workflows/web-preview.yml` builds the Web export on every pull request and push to `main` and deploys it to Cloudflare Pages, commenting the playable link on the pull request; it is not a required check. It pins the same Godot version as `verify.yml`, so update both together. See the README's Web export section for the 25 MiB file limit and the secrets it needs.
