...
- `tools/sim_runs.gd` plays seeded runs headless and prints the wave, game time, kills, Cash and Coins each ended with, and the median; `-- --seeds N` and `-- --cap-minutes N` narrow it, `-- --buy none|cheapest|even|attack` picks how it spends Cash, and `-- --careers N` plays N runs in a row from a fresh Workshop, spending Coins between them. It is a measurement tool, not a gate.
